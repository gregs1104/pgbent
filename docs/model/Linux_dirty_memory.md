...
A view into the performance study is available on the Streamlit app at https://pgbent.streamlit.app/ under the option "OSM Dirty Memory".
...
Since this test server is using a fast SSD, there is a small performance win but not a lot of value to caching large amounts of Dirty memory:
